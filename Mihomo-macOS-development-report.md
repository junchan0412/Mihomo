...
设置使用标准 `Settings` scene，`Command-,` 打开独立设置窗口。主侧栏的设置按钮通过 `OpenSettingsAction` 执行同一系统动作，不再把设置伪装成主内容区页面。

主窗口使用原生 Source List 与可自定义 Toolbar。导航、搜索、刷新和常用控制通过菜单命令与 `FocusedValues` 路由到当前工作区；不得用全局单例 selection 绕过窗口上下文。Profile 编辑器使用 value-based `WindowGroup`，允许多个配置窗口独立恢复和关闭。
...
控制通道是例外：`external-controller` 与 `secret` 在所有合并完成后由应用强制写入，Profile、JS Transform 和 YAML 覆写不能改变客户端连接自己启动核心所需的地址与密钥。域名嗅探在 `snifferManagedByApp` 开启时同样由应用管理；关闭后才遵循标准合并优先级。
...
APP_VERSION=1.10.0 ./script/build_and_run.sh --verify
```
...
7. 提交并 push branch。
8. 创建 `v1.10.0` tag，不移动旧 tag。
9. push tag。
10. 创建 GitHub Release，上传 zip、versioned manifest、`mihomo-update.json` 与 provenance，正文使用 `docs/releases/v1.10.0.md`。
