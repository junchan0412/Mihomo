...
- 菜单栏图标改回 M 字形品牌标记，模式字母（R/D/G）叠加在右下角，颜色随模式变化。
- 修复菜单栏速率开关与设置页不同步的问题，统一使用 AppSettings 管理。
- 为 9 个主页面补齐 pageBackground，消除页面间背景色不一致的视觉突兀。

完整变更见 [v1.15.0 Release Notes](docs/releases/v1.15.0.md)。
