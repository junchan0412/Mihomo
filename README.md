...
- 重新设计配置页、覆写管理与配置质量面板，明确最终优先级：`YAML 覆写 > JS Transform > Profile 配置 > 应用默认`。
...
- 策略页增加 GUI 策略组编辑器；规则页提供明显的 GUI 新建、查看、编辑和删除入口。
- Provider 批量更新同时处理远程与本地资源；本地文件会执行路径、存在性、非空和可读性校验。
- 资源详情隐藏远程 URL query/fragment，避免订阅 token 出现在界面中。

完整变更见 [v1.8.77 Release Notes](docs/releases/v1.8.77.md)，架构与开发约定见 [开发文档](Mihomo-macOS-development-report.md)。
...
当前测试集包含 91 个 XCTest，覆盖设置迁移、Runtime Config 合并、Profile 结构编辑、Provider 更新与回滚、网络请求超时、Controller WebSocket 恢复、Helper 路径边界、备份恢复、更新回滚、Secret Vault 和 AppKit accessibility。
...
- `Mihomo-1.8.77-macOS-arm64.zip`
- `Mihomo-1.8.77-update.json`
- `mihomo-update.json`
- `Mihomo-1.8.77-provenance.md`
