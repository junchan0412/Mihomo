...
- 资源页进入与“配置资源 / 节点提供商 / 规则”内部切换统一使用渐入渐出过渡，并固定工作区与计数区域尺寸，减少切换时的轻微布局位移。
- 菜单栏图标改为模式感知的紧凑状态徽标；运行状态、出站模式和上传下载速率在 19pt 菜单栏空间内保持稳定对齐。
- 菜单栏下拉菜单重构为参考 macOS 弹窗风格的分组列表，加入左侧图标、右侧状态、chevron、快捷键提示、配置切换和面板入口。
- 菜单栏“显示上传下载速率”开关改为持久化保存，最近切换分组仅在有可用记录时显示。

完整变更见 [v1.25.4 Release Notes](docs/releases/v1.25.4.md)。
