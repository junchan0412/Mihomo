...
final class ConfigFragmentStore {
    ...
    func parseProviders(profileContent: String) -> [ProviderItem] {
        parseProviderBlock(kind: "Proxy", key: "proxy-providers", in: profileContent)
...
final class LaunchDaemonManager {
    private let label = "dev.codex.Mihomo.core"

    var plistPath: String {
        "/Library/LaunchDaemons/\(label).plist"
    }

    func install(corePath: String, configPath: URL, workDirectory: URL) throws -> String {
        try AppPaths.ensureBaseDirectories()
        let plist = launchDaemonPlist(corePath: corePath, configPath: configPath, workDirectory: workDirectory)
        let temp = AppPaths.runtimeDirectory.appendingPathComponent("\(label).plist")
        try plist.write(to: temp, atomically: true, encoding: .utf8)
        let script = """
        /bin/mkdir -p /Library/LaunchDaemons
        /bin/cp \(PrivilegedShell.shellQuote(temp.path)) \(PrivilegedShell.shellQuote(plistPath))
        /usr/sbin/chown root:wheel \(PrivilegedShell.shellQuote(plistPath))
        /bin/chmod 644 \(PrivilegedShell.shellQuote(plistPath))
        /bin/launchctl bootout system \(PrivilegedShell.shellQuote(plistPath)) >/dev/null 2>&1 || true
        /bin/launchctl bootstrap system \(PrivilegedShell.shellQuote(plistPath))
        """
        _ = try PrivilegedShell.run(script)
        return plistPath
    }
    ...
    func uninstall() throws {
        let script = """
        /bin/launchctl bootout system \(PrivilegedShell.shellQuote(plistPath)) >/dev/null 2>&1 || true
        /bin/rm -f \(PrivilegedShell.shellQuote(plistPath))
        """
        _ = try PrivilegedShell.run(script)
    }

    private func launchDaemonPlist(corePath: String, configPath: URL, workDirectory: URL) -> String {
        """
        ...
        <dict>
          ...
          <array>
            ...
            <string>-d</string>
            <string>\(xml(workDirectory.path))</string>
            <string>-f</string>
            <string>\(xml(configPath.path))</string>
          </array>
          ...
          <string>\(xml(AppPaths.coreLogFile.path))</string>
