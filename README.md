...
- Local, remote, drag-and-drop, and `mihomo://` deep-link Profile import, queued remote subscription refresh, automatic refresh interval, failure notifications, certificate fingerprint pinning, a built-in YAML editor, and structured UI editing for policy groups and rules.
...
- Rule table with persisted disabled-rule filtering for generated runtime config and live hit counts from Controller connections.
...
- Fixed ad-hoc signing identifiers for the app, Helper, and bundled mihomo core, release manifest generation, and a verified in-app updater using manifest URL, Ed25519 manifest signature, SHA-256, bundle id, and signing identifier checks.
...
```bash
./script/package_release.sh 0.8.0
```
...
For later in-app updates, upload the zip and manifest to the same release/static directory, set the Advanced page's manifest URL to `mihomo-update.json`, then use Check/Install Update inside Mihomo.
