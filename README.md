...
- 控制器轮询拆分：实时 WebSocket 健康时降低连接轮询与元数据刷新频率。
- AppKit 表格差分刷新与连接行缓存，显著减少高频流量下的界面闪烁。
- 统一交互动画节奏，侧栏/主分区切换更顺滑，指标数字过渡更稳定。

完整变更见 [v1.16.0 Release Notes](docs/releases/v1.16.0.md)。
