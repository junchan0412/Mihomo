...
jq -n \
  ...
  --arg notes "$NOTES" \
  '{version: $version, build: $build, url: $url, sha256: $sha256, minimumSystemVersion: $minimumSystemVersion, bundleIdentifier: $bundleIdentifier, signingIdentifier: $signingIdentifier, publishedAt: $publishedAt, notes: $notes}' \
  >"$MANIFEST_PATH"
